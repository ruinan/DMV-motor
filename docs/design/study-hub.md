...
2. **Difficulty judge rubric** — ~~"Hard" means *all four choices look defensible at first glance*; a question with even one obviously-wrong distractor fails.~~ **Revised 2026-05-28 (user-approved during B4):** the original distractor-plausibility gate rejected legitimate safety-procedure questions where the wrong answers are *intentionally dangerous* (e.g. "to fix a stuck throttle, accelerate harder" — obviously wrong, but a valid M1 distractor). New binary rubric: **pass=true if answering correctly requires DMV-handbook knowledge or real motorcycle experience; pass=false only if the correct answer is derivable from pure common sense (no handbook needed) or the question is trivially worded.** Dangerous distractors are explicitly allowed. This is the gate the V16 question bank was filtered through. Implemented in `aiqgen/application/DifficultyJudge.java`.
